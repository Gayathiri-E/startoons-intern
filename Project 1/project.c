...
void find_and_plot_peaks(double *data, int size, const char *label) {
    int peak_indices[MAX_SIZE], num_peaks;
    double inverted_data[MAX_SIZE];

    // Find maxima (peaks)
    find_peaks(data, size, peak_indices, &num_peaks, DISTANCE, PROMINENCE);

    // Find minima by inverting the signal and finding peaks
    for (int i = 0; i < size; i++) {
        inverted_data[i] = -data[i];
    }
    int minima_indices[MAX_SIZE], num_minima;
    find_peaks(inverted_data, size, minima_indices, &num_minima, DISTANCE, PROMINENCE);

    // Plot the data, peaks, and minima using ASCII art
    plot_ascii(data, size, peak_indices, num_peaks, minima_indices, num_minima, label);
}
